// ESPHome external component: OBD-II BLE client — NimBLE-Arduino 1.4.1 impl
#include "obd_ble.h"
...
static const int SCAN_DURATION_SEC = 15;
...
static const int G6_BMS_C = sizeof(G6_BMS)/sizeof(OBDPidDef);
static const int G6_VCU_C = sizeof(G6_VCU)/sizeof(OBDPidDef);
static const char* ELM_INIT[] = {"AT Z","AT E0","AT L0","AT S0","AT H1","AT SP6","AT M0","AT AT1","AT FCSM1",
  "AT SH 704","AT CRA 784","AT FCSH 704"};
static const int ELM_INIT_C = sizeof(ELM_INIT)/sizeof(char*);

// ── Helpers ────────────────────────────────────────────────────────────
static std::vector<int> hex_to_bytes(const std::string& h) {
  std::vector<int> b;
...
void OBDComponent::update(){
  if(state_==PollState::IDLE && client_ && client_->isConnected()){
    poll_cycle_++; current_pid_index_=0; bms_done_=false;
    state_=PollState::POLL_BMS; state_start_ms_=millis();
  }
}
...
void OBDComponent::loop(){
  ...
  switch(state_){
    case PollState::IDLE:
      if(!client_||!client_->isConnected()){start_scan();state_=PollState::SCANNING;state_start_ms_=now;}
      break;
    case PollState::SCANNING:
      // Wait for scan to complete (non-blocking: check every loop())
      {NimBLEScan* scan=NimBLEDevice::getScan();
      if(!scan->isScanning()||now-state_start_ms_>(SCAN_DURATION_SEC+5)*1000){
        if(process_scan()){state_=PollState::CONNECTING;state_start_ms_=now;}
        else{ESP_LOGW(TAG,"Device not found, retry %dms",RECONNECT_DELAY_MS);state_=PollState::IDLE;state_start_ms_=now;}
      }}
      break;
    case PollState::CONNECTING:
      if(target_device_&&now-state_start_ms_>2000){
        if(connect_ble(target_device_)){
          state_=PollState::DISCOVERING;
        } else {
          ESP_LOGW(TAG,"Connection failed"); state_=PollState::IDLE;
        }
      }
      break;
    case PollState::DISCOVERING:
      discover_services();
      if(char_write_&&char_notify_){ESP_LOGI(TAG,"GATT ready, init ELM327");init_cmd_index_=0;state_=PollState::INIT_ELM;}
      else{ESP_LOGW(TAG,"Discovery fail");state_=PollState::IDLE;}
      break;
    case PollState::INIT_ELM:
      if(init_cmd_index_>=ELM_INIT_C){ESP_LOGI(TAG,"ELM327 ready");current_ecu_="bms";state_=PollState::IDLE;break;}
      send_at_command(ELM_INIT[init_cmd_index_]);current_command_=ELM_INIT[init_cmd_index_];
      state_=PollState::INIT_WAIT;state_start_ms_=now;break;
    case PollState::INIT_WAIT:
      if(now-state_start_ms_>CMD_TIMEOUT_MS){init_cmd_index_++;state_=PollState::INIT_ELM;}break;
    ...
    case PollState::WAIT_RESPONSE:
      if(now-state_start_ms_>CMD_TIMEOUT_MS){current_pid_index_++;
        state_=(bms_done_&&current_pid_index_>=pids_.size())?PollState::IDLE:(bms_done_?PollState::POLL_VCU:PollState::POLL_BMS);}
      break;
  }
}
...
void OBDComponent::start_scan(){
  ...
  scan->start(SCAN_DURATION_SEC,false);
  ESP_LOGI(TAG,"Scan started (%ds)...",SCAN_DURATION_SEC);
}

bool OBDComponent::process_scan(){
  NimBLEScanResults r=NimBLEDevice::getScan()->getResults();
  ESP_LOGI(TAG,"Scan: %d devices",r.getCount());
  std::string tm=mac_address_;tm.erase(std::remove(tm.begin(),tm.end(),':'),tm.end());
...
bool OBDComponent::connect_ble(NimBLEAdvertisedDevice* device){
...
void OBDComponent::on_notify(uint8_t* data,size_t len){
  for(size_t i=0;i<len;i++){
    if(data[i]==ELM_PROMPT){
      std::string c;for(char ch:rx_buffer_)if(ch!=' '&&ch!='\r'&&ch!='\n'&&ch!='\t'&&ch!='>')c+=ch;
      rx_buffer_.clear();if(c.empty())return;
...
bool OBDComponent::send_at_command(const std::string& cmd){
  if(!char_write_)return false;
  std::string p=cmd+"\r";
  return char_write_->writeValue((uint8_t*)p.c_str(),p.length(),false);
}
...
float OBDComponent::parse_response(const std::string& r,const std::string& f){
  ...
  auto b=hex_to_bytes(c);if(b.size()<2)return NAN;if(b.size()>=3&&b[1]==0x7F)return NAN;
